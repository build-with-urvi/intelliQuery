...
- A message combining a real item with a bare CATEGORY word ("garlic
  bread and pizza") adds the item and then shows that category's list
  (see _extract_leftover_category) — but a message combining an item
  with a SPECIFIC pizza/item name ("garlic bread and veggie paradise")
  still only acts on whichever one find_pizza()/find_menu_item() matches
  first against the whole message; it does not add both. Only the
  item+bare-category combo is specifically handled.
"""
...
# ---------------------------------------------------------------------------
# "Item + bare category" combo messages — e.g. "garlic bread and pizza".
# find_menu_item()'s substring matching already grabs "Garlic Bread" out
# of that whole message (see _matches), but the leftover "and pizza" has
# no real menu item literally called "pizza" — it's a CATEGORY — so
# without this it was silently dropped, adding only the garlic bread.
#
# This is checked ONLY after a confident item match, and ONLY looks for a
# bare CATEGORY word in what's left over — never a specific pizza/item
# name. A message naming a real pizza directly (e.g. "garlic bread and
# veggie paradise") never reaches this: find_pizza() is checked before
# find_menu_item() everywhere in handle_order_message(), so "veggie
# paradise" is already matched and handled as a real pizza first.
# ---------------------------------------------------------------------------

def _extract_leftover_category(user_text: str, matched_name: str):
    """After `user_text` has already matched a real menu item named
    `matched_name`, strip the matched item's own words out and check
    whether what's left is a bare category word (see
    GENERIC_CATEGORY_WORDS) — e.g. 'garlic bread and pizza' -> leftover
    'and pizza' -> category 'Pizzas'. Returns the category key or None."""
    leftover = user_text.lower()
    for word in _tokenize(matched_name.lower()):
        leftover = re.sub(rf"\b{re.escape(word)}\b", " ", leftover)
    return _detect_generic_category(leftover)
...
def handle_order_message(tenant: str, phone_number: str, user_text: str):
        ...
        if not looks_like_a_question:
            ...
            if item_match:
                ...
                if _tokens_confidently_match(user_text, name):
                    ...
                    # e.g. "garlic bread and pizza" as the very first
                    # message — see _extract_leftover_category.
                    leftover_category = _extract_leftover_category(user_text, name)
                    if leftover_category:
                        return (
                            f"Added {name} (Rs.{price}).\n\n"
                            f"{_current_cart_summary(cart)}\n\n"
                            f"{format_category_list(tenant, leftover_category)}"
                        )
    ...
    if step == STEP_COLLECTING:
        ...
        if _tokens_confidently_match(user_text, name):
            ...
            # e.g. "garlic bread and pizza" — Garlic Bread matched above
            # via substring/token matching, but "pizza" isn't a literal
            # menu item, it's a CATEGORY. Without this the leftover
            # "pizza" is silently dropped. Show that category's items so
            # the customer can pick a specific one next; we stay in
            # STEP_COLLECTING, so the next message (e.g. "veggie
            # paradise") hits find_pizza() as usual and flows straight
            # into the normal topping/size steps, landing in this same
            # cart alongside the item just added.
            leftover_category = _extract_leftover_category(user_text, name)
            if leftover_category:
                return (
                    f"Added {name} (Rs.{price}).\n\n"
